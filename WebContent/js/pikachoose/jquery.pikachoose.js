	...
	$pc.fn.extend({
		...
		*/
		finishAnimating: function(data){
			...
			//动画结束后发出imgswitch事件并带出img的id
			jQuery.event.trigger('imgswitch',this.active[0].id);
			//alert(this.image.id);
		...
		*/
		...
		nextClick : function(e){
			...
			//此处出发imgswitch事件,并把图片id传出去
			//jQuery.event.trigger('imgswitch',next[0].id);
		...
		prevClick : function(e){
			...
			//此处出发imgswitch事件,并把图片id传出去
			//jQuery.event.trigger('imgswitch',prev[0].id);
		},
